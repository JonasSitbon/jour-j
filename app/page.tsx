...
function Hero() {
  return (
    <section className="relative h-screen flex flex-col items-center justify-center overflow-hidden"
      ...
      {/* Background: subtle radial warmth, no blobs or shapes */}
      <div className="absolute inset-0 pointer-events-none"
        style={{ background: `radial-gradient(ellipse 80% 60% at 50% 40%, ${WARM_SOFT} 0%, ${BG_CREAM} 100%)` }} />

      {/* Top rule */}
      <div className="absolute top-0 left-0 right-0 h-px" style={{ background: `rgba(201,110,44,0.18)` }} />
      ...
      <div className="relative z-10 flex flex-col items-center text-center px-6 max-w-4xl">
        ...
        <motion.p
          ...
          transition={{ duration: 0.5, ease: [0.16, 1, 0.3, 1] }}
          className="text-[11px] font-semibold uppercase tracking-[0.28em] mb-10"
          style={{ color: TC }}>
          Wedding Planning Platform
        </motion.p>

        {/* Headline — big, simple, centered */}
        <motion.h1
          className="font-bold leading-[1.0] tracking-[-0.04em] mb-8"
          style={{ fontSize: "clamp(3.2rem, 10vw, 7.5rem)", color: TEXT_DARK }}
          initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.2 }}>
          {["Votre", "mariage,", "organisé."].map((word, i) => (
            <motion.span key={i}
              ...
              className="block">
              {i === 1
                ? <>{word.slice(0, -1)}<span style={{ color: TC }}>,</span></>
                : i === 2
                ? <span className="font-light italic" style={{ color: TEXT_MID }}>{word}</span>
                : word}
            </motion.span>
          ))}
        </motion.h1>
        ...
        <motion.p
          ...
          className="text-[1.05rem] leading-relaxed max-w-md mb-10"
          style={{ color: TEXT_MID }}>
          Invités · Budget · Prestataires · Jour J · Plan de table.
          <br />Tout dans un seul espace, gratuit.
        </motion.p>
        ...
        <motion.div
          ...
          <Link href="/signup"
            className="px-8 py-3.5 text-[14px] font-semibold text-white tracking-wide transition-all hover:opacity-90 active:scale-[0.97]"
            style={{ background: TEXT_DARK }}>
            Commencer — c&apos;est gratuit
          </Link>
          <a href="#demo"
            className="text-[14px] font-medium tracking-wide transition-colors group flex items-center gap-2"
            style={{ color: TEXT_MID }}
      ...
      {/* Bottom: thin rule + stats bar */}
      <motion.div
        initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.1, duration: 0.6 }}
        className="absolute bottom-0 left-0 right-0">
        <div className="border-t" style={{ borderColor: "rgba(201,110,44,0.12)" }}>
          <div className="max-w-4xl mx-auto px-6 py-5 flex items-center justify-center flex-wrap gap-8 sm:gap-14">
            {[["2 400+", "mariages planifiés"], ["16", "modules inclus"], ["100%", "gratuit"], ["4.9 ★", "satisfaction"]].map(([v, l]) => (
              <div key={l} className="text-center">
                <div className="text-[15px] font-bold" style={{ color: TEXT_DARK }}>{v}</div>
                <div className="text-[10px] uppercase tracking-[0.12em]" style={{ color: TEXT_LIGHT }}>{l}</div>
              </div>
            ))}
          </div>
        </div>
