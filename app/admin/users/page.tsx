...
export default function AdminUsersPage() {
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-2xl font-bold mb-1" style={{ color: "#f0ead8" }}>Utilisateurs</h1>
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 mb-5">
        <div className="relative flex-1 min-w-0">
          <span className="absolute left-3 top-1/2 -translate-y-1/2" style={{ color: "#4b5563" }}><Icon name="search" size={15} /></span>
          <input
            type="text"
            value={search}
            ...
            style={{ background: "#1a1a2e", borderColor: "#2a2a3e", color: "#d1cec8" }}
          />
        </div>
        <div className="flex items-center gap-2">
          <span className="text-xs" style={{ color: "#6b7280" }}>Trier par</span>
          <div className="flex rounded-lg overflow-hidden border" style={{ borderColor: "#2a2a3e" }}>
            {(["date", "name"] as const).map((s) => (
              <button
                ...
                style={{
                  background: sortBy === s ? "#C96E2C22" : "#1a1a2e",
                  color: sortBy === s ? "#e2945a" : "#6b7280",
                }}
              >
                {s === "date" ? "Date" : "Nom"}
              </button>
        ...
        <div className="text-xs px-3 py-1.5 rounded-lg" style={{ background: "#0f1117", color: "#4b5563" }}>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr style={{ borderBottom: "1px solid #2a2a3e" }}>
                <th className="text-left px-4 py-3 text-xs font-semibold" style={{ color: "#4b5563" }}>Utilisateur</th>
                <th className="text-left px-4 py-3 text-xs font-semibold" style={{ color: "#4b5563" }}>ID</th>
                <th className="text-left px-4 py-3 text-xs font-semibold" style={{ color: "#4b5563" }}>Type de compte</th>
                <th className="text-left px-4 py-3 text-xs font-semibold" style={{ color: "#4b5563" }}>Inscription</th>
                <th className="text-left px-4 py-3 text-xs font-semibold" style={{ color: "#4b5563" }}>Changer type</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan={5} className="text-center py-12 text-sm" style={{ color: "#4b5563" }}>
                    Aucun utilisateur trouvé
                  </td>
                </tr>
              ) : paged.map((u, i) => {
                const name = [u.first_name, u.last_name].filter(Boolean).join(" ") || "Sans nom";
                const badge = ACCOUNT_BADGE[u.account_type] ?? { label: u.account_type, bg: "#33333344", color: "#9ca3af" };
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2.5">
                        ...
                        <span className="font-medium text-[13px]" style={{ color: "#d1cec8" }}>{name}</span>
                      </div>
                    </td>
                    {/* ID */}
                    <td className="px-4 py-3">
