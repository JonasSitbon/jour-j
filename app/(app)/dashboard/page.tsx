...
/* ------------------------------------------------------------------ */
/* GettingStarted — kept exactly as-is                                 */
/* ------------------------------------------------------------------ */
    ...
    <div className="mb-5 rounded-card border border-line bg-surface p-5 md:p-6">
      ...
      <div className="flex flex-wrap gap-2">
        {steps.map((s, i) => {
          ...
          const content = (
            <span
              className={`inline-flex items-center gap-2 px-3.5 py-2 rounded-lg border text-[12.5px] font-medium transition-all ${
                s.done
                  ? "border-sage/40 bg-sage-soft/60 text-sage cursor-default"
                  : isNext
                  ? "border-primary bg-primary-soft text-primary-700 hover:shadow-xs hover:-translate-y-px"
                  : "border-line bg-surface-2 text-text-3 cursor-default"
              }`}
            >
              <span
                className={`w-[19px] h-[19px] rounded-full flex items-center justify-center text-[10px] font-bold shrink-0 ${
                  s.done
                    ? "bg-sage text-white"
                    : isNext
              ...
              <Icon
                ...
                size={13}
                className={
                  s.done ? "text-sage" : isNext ? "text-primary" : "text-text-3"
                }
              />
              {s.label}
              {isNext && (
                <Icon name="chevronR" size={12} className="ml-0.5" />
              )}
            </span>
          );
...
/* ------------------------------------------------------------------ */
/* ScoreWidget — Ring + 4 mini progress bars                           */
/* ------------------------------------------------------------------ */
      ...
      <div className="flex flex-col items-center gap-1">
        <Ring value={globalPct} size={120} stroke={11} color="var(--primary)" track="var(--surface-3)">
          <div className="flex flex-col items-center leading-none">
            <span className="font-mono text-[30px] font-bold tracking-tight text-text">
              {globalPct}
            </span>
        ...
        {bars.map(({ label, pct, color }) => (
          <div key={label}>
            <div className="flex items-center justify-between mb-1.5">
              ...
              <span className="font-mono text-[12px] font-semibold text-text-2">
                {pct}%
              </span>
            </div>
...
/* ------------------------------------------------------------------ */
/* DeadlineItem — colored left border card                             */
/* ------------------------------------------------------------------ */
type DeadlineEntry = {
  date: string;
  title: string;
...
function DeadlineItem({ item, isLast }: { item: DeadlineEntry; isLast: boolean }) {
  ...
  const badgeEl = isLate ? (
    <span className="inline-flex items-center gap-1 text-[11px] font-semibold text-coral bg-coral-soft px-2 py-0.5 rounded-full">
      <Icon name="alert" size={10} />
      En retard
    </span>
  ) : isSoon ? (
    <span className="inline-flex items-center gap-1 text-[11px] font-semibold text-[var(--gold-ink)] bg-amber-soft px-2 py-0.5 rounded-full">
      <Icon name="clock" size={10} />
      dans {d}j
    </span>
  ) : (
    <span className="text-[11px] font-mono text-text-3">
      {fmt.date(item.date, { day: "numeric", month: "short" })}
    </span>
  );

  return (
    <div
      className={`flex items-start gap-3.5 px-4 py-3.5 border-l-[3px] bg-surface rounded-r-md ${borderColor} ${
        !isLast ? "mb-2" : ""
      }`}
    >
      <div
        className={`w-8 h-8 rounded-[9px] flex items-center justify-center shrink-0 ${
          isLate
            ...
            : item.type === "pay"
            ? "bg-gold-soft text-[var(--gold-ink)]"
...
/* ------------------------------------------------------------------ */
/* AlertItem — colored left border with "Résoudre" CTA                */
/* ------------------------------------------------------------------ */
type AlertEntry = {
  tone: string;
  icon: string;
...
function AlertItem({ alert, isLast }: { alert: AlertEntry; isLast: boolean }) {
  ...
  return (
    <div
      className={`flex items-start gap-3.5 px-4 py-3.5 border-l-[3px] bg-surface rounded-r-md ${border} ${
        !isLast ? "mb-2" : ""
      }`}
    >
      <div
        className={`w-8 h-8 rounded-[9px] flex items-center justify-center shrink-0 ${
          TILE_TONE[alert.tone] ?? TILE_TONE.primary
        }`}
      >
      ...
      <Link
        href={`/${alert.go}`}
        className="shrink-0 text-[12px] font-semibold text-primary hover:underline whitespace-nowrap flex items-center gap-1 pt-0.5"
      >
        Résoudre
...
/* ------------------------------------------------------------------ */
/* CountdownRing — animated SVG ring around J-days number             */
/* ------------------------------------------------------------------ */
function CountdownRing({ days }: { days: number }) {
  ...
  // Planning horizon: 730 days (2 years)
  const TOTAL_DAYS = 730;
  // Progress = how far along we are (days elapsed out of TOTAL_DAYS)
  ...
  // Color: sage (early) → terracotta/coral (close to date)
  // pct close to 1 means wedding is near
  const strokeColor =
    pct < 0.5
      ? "var(--sage)"
      : pct < 0.85
...
/* ------------------------------------------------------------------ */
/* FocusItem — single row in "Focus du jour"                          */
/* ------------------------------------------------------------------ */
type FocusItem = {
...
const FOCUS_BORDER: Record<FocusItem["tone"], string> = {
  coral:   "border-l-coral",
  gold:    "border-l-[var(--gold)]",
  primary: "border-l-primary",
};

const FOCUS_ICON_BG: Record<FocusItem["tone"], string> = {
  coral:   "bg-coral-soft text-coral",
  gold:    "bg-gold-soft text-[var(--gold-ink)]",
  primary: "bg-primary-soft text-primary-700",
};
...
function FocusItemRow({ item, isLast }: { item: FocusItem; isLast: boolean }) {
  return (
    <div
      className={`flex items-center gap-3.5 px-4 py-3 border-l-[3px] bg-surface rounded-r-md ${FOCUS_BORDER[item.tone]} ${
        !isLast ? "mb-2" : ""
      }`}
    >
      ...
      <div className="flex-1 min-w-0 text-[13px] font-medium leading-snug truncate">
        {item.label}
      </div>
      <Link
        href={item.href}
        className="shrink-0 text-[12px] font-semibold text-primary hover:underline whitespace-nowrap flex items-center gap-0.5"
      >
        Voir
        <Icon name="chevronR" size={12} />
      </Link>
...
export default function DashboardPage() {
  ...
  const modules = [
    {
      id: "guests", icon: "users", tone: "primary", title: "Invités",
      val:   state.guests.length === 0 ? "À remplir"   : `${confirmed} / ${state.guests.length}`,
      sub:   state.guests.length === 0 ? "Ajoutez vos invités"   : `${pending} en attente`,
      empty: state.guests.length === 0,
    },
    {
      id: "budget", icon: "wallet", tone: "sage", title: "Budget",
      val:   state.budget.length === 0 ? "À définir"   : fmt.eur(spent),
      sub:   state.budget.length === 0 ? "Configurez les postes" : `${pctSpent}% engagé`,
      empty: state.budget.length === 0,
    },
    {
      id: "vendors", icon: "file", tone: "gold", title: "Devis",
      val:   state.vendors.length === 0 ? "À contacter" : `${signed} signés`,
      sub:   state.vendors.length === 0 ? "Comparez les devis"   : `${pendingV} à arbitrer`,
      empty: state.vendors.length === 0,
    },
    {
      id: "payments", icon: "card", tone: latePayments.length ? "coral" : "blue", title: "Paiements",
      val:   state.payments.length === 0 ? "À suivre"   : fmt.eur(dueAmt),
      sub:   state.payments.length === 0 ? "Suivi des échéances" : latePayments.length ? `${latePayments.length} en retard` : "à venir",
      empty: state.payments.length === 0,
    },
    {
      id: "dates", icon: "calendar", tone: "amber", title: "Date",
      val:   fmt.dateShort(w.date),
      sub:   dateCand ? `${dateCand.weather}% météo` : "—",
      empty: false,
    },
    {
      id: "checklist", icon: "check-circle", tone: "primary", title: "Checklist",
      val:   state.tasks.length === 0 ? "À créer"     : `${doneTasks} / ${state.tasks.length}`,
      sub:   state.tasks.length === 0 ? "Initialisez la checklist" : `${pctTasks}% fait`,
  ...
  /* ---- Upcoming deadlines (max 6, late first) ---- */
  const upcoming = useMemo<DeadlineEntry[]>(() => {
    const list: DeadlineEntry[] = [
      ...state.payments.filter((p) => p.status !== "paid").map((p) => ({
        date: p.due, title: p.label, meta: `${p.vendor} · ${fmt.eur(p.amount)}`, type: "pay" as const, status: p.status,
      })),
      ...state.tasks.filter((t) => !t.done && t.due).map((t) => ({
        date: t.due, title: t.title, meta: `Tâche · ${t.who === "A" ? w.partnerA : w.partnerB}`, type: "task" as const, status: "",
      })),
    ];
    return list
      .sort((a, b) => {
  ...
  /* ---- Focus du jour (top 3 most urgent items) ---- */
  const focusItems = useMemo<FocusItem[]>(() => {
    ...
    for (const p of state.payments) {
      if (p.status === "late") {
        list.push({
          tone: "coral",
          icon: "alert",
          label: `Paiement en retard · ${p.vendor} — ${p.amount}€`,
          href: "/payments",
        });
      }
    }
    ...
    for (const t of state.tasks) {
      if (!t.done && t.due && t.due < todayIso) {
        list.push({
          tone: "coral",
          icon: "alert",
          label: `Tâche en retard · ${t.title}`,
          href: "/checklist",
        });
      }
    }

    // 3. Payments due within 7 days
    for (const p of state.payments) {
      if (p.status !== "paid") {
        ...
        if (d >= 0 && d <= 7) {
          list.push({
            tone: "gold",
            icon: "clock",
            label: `Paiement dans ${d}j · ${p.vendor} — ${p.amount}€`,
            href: "/payments",
          });
        }
      }
    ...
    // 4. Guests without RSVP when wedding < 21 days away
    if (days < 21 && days >= 0 && pending > 0) {
      list.push({
        tone: "primary",
        icon: "users",
        label: `${pending} invité${pending > 1 ? "s" : ""} sans réponse — J-${days}`,
        href: "/guests",
      });
    }
    ...
    for (const t of state.tasks) {
      if (!t.done && t.due && t.due >= todayIso && t.due <= in7Iso) {
        list.push({
          tone: "primary",
          icon: "clock",
          label: `Tâche cette semaine · ${t.title}`,
          href: "/checklist",
        });
      }
    }
  ...
  return (
    <div className="mx-auto w-full max-w-[1320px] px-5 md:px-8 py-6 md:py-8 pb-28 md:pb-12">
      ...
      {/* ---- Greeting line ---- */}
      {greeting && (
        <div className="text-[14px] text-text-2 font-medium mb-1 -mt-1">
          {greeting}
        </div>
      )}

      <PageHead
        title={isNewUser ? <>Bienvenue dans Jour J</> : <>Bonjour {w.partnerA}</>}
        sub={
          isNewUser
            ? "Votre espace est prêt. Suivez les étapes ci-dessous pour organiser votre mariage."
            : "Voici où en sont vos préparatifs aujourd'hui."
        }
        actions={
          <>
            <Button variant="secondary" icon="download">Exporter</Button>
            <Link href="/vendors">
              <Button variant="primary" icon="plus">Ajouter un devis</Button>
            </Link>
          </>
        }
      />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1.6fr_1fr] gap-5 items-stretch mb-5">
        ...
        {/* Hero card */}
        <div
          className="relative overflow-hidden rounded-card shadow-md flex flex-col justify-between min-h-[268px]"
          style={{
            background:
              "radial-gradient(120% 130% at 100% 0%, color-mix(in srgb, var(--primary) 88%, #fff), transparent 58%), radial-gradient(90% 120% at 0% 100%, color-mix(in srgb, var(--gold) 60%, #6E4423), transparent 60%), linear-gradient(125deg, #4A3320 0%, #6E4423 100%)",
          }}
        >
          {/* % prêt badge — top right */}
          ...
          {/* Top: couple names */}
          <div className="relative z-[1] px-7 md:px-8 pt-7 md:pt-8">
            <div className="text-[12.5px] text-[#FBF1E0]/80 font-medium flex items-center gap-2 mb-2">
              <Icon name="rings" size={15} />
              Le mariage de
            </div>
            <div className="text-[28px] md:text-[32px] font-semibold tracking-[-.025em] text-[#FBF1E0] leading-tight">
          ...
          {/* Bottom: countdown or "grand jour passé" */}
          <div className="relative z-[1] px-7 md:px-8 pb-7 md:pb-8 flex items-end justify-between gap-4 mt-6">
            {isPast ? (
              ...
              <div className="flex items-end gap-5">
                {/* Animated ring around the countdown */}
                <CountdownRing days={days} />
                <div>
                  <div className="text-[#FBF1E0]/80 text-[12px] font-semibold uppercase tracking-widest mb-1">
                    Compte à rebours
                  </div>
                  <div className="text-[#FBF1E0]/70 text-[13px] mt-1">
                    {days === 1 ? "jour avant le grand jour" : "jours avant le grand jour"}
                  </div>
                </div>
              </div>
            )}
            ...
            {dateCand && !isPast && (
              <div className="flex items-center gap-3 bg-white/12 border border-white/20 rounded-xl px-4 py-3 backdrop-blur-sm shrink-0">
                ...
                <div>
                  <div className="font-mono text-[24px] font-bold text-[#FBF1E0] leading-none">
                    {dateCand.temp}°
                  </div>
                  <div className="text-[11.5px] text-[#FBF1E0]/80 mt-0.5">
                    {dateCand.weather}% favorable
                  </div>
                </div>
              </div>
            )}
        ...
        {focusItems.length === 0 ? (
          ...
          <div>
            {focusItems.map((item, i) => (
              <FocusItemRow key={i} item={item} isLast={i === focusItems.length - 1} />
            ))}
          </div>
        )}
      </div>
      ...
      <div className="flex flex-wrap gap-2.5 mb-8">
        {(
          ...
          ] as { icon: string; label: string; href: string }[]
        ).map((action) => (
          <Link key={action.href + action.label} href={action.href}>
            <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-coral text-coral text-[13px] font-medium hover:bg-primary-soft hover:border-primary hover:text-primary-700 transition-all cursor-pointer select-none">
      ...
      {/* ---- Finance bar (full width below bento top) ---- */}
      <Card className="mb-5">
        <div className="flex items-center justify-between mb-3">
          ...
          <Link href="/budget" className="link text-[13px]">
            Voir le détail
          </Link>
        </div>
        <div className="h-3 rounded-full overflow-hidden bg-surface-3 mb-4">
          <div className="flex h-full">
            <span
              className="block h-full bg-sage transition-[width] duration-700 rounded-l-full"
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="sec-title">
          <Icon name="grid" size={17} className="text-text-3" />
          Modules
        </div>
      </div>
        ...
        {modules.map((m) => (
          <Link key={m.id} href={`/${m.id}`}>
            <Card
              hover
              pad={false}
              className={`p-4 flex flex-col gap-3 relative h-full group ${
                m.empty ? "border-dashed" : ""
              }`}
            >
              {/* Arrow */}
              <Icon
              ...
              <div>
                ...
                {m.empty ? (
                  ...
                  <>
                    <div className="font-mono text-[20px] font-bold tracking-[-.015em] leading-tight">
                      {m.val}
                    </div>
              ...
              {m.empty && (
                <span className="absolute bottom-3.5 right-3.5 w-2 h-2 rounded-full bg-amber-400 animate-pulse" />
              )}
            </Card>
          </Link>
        ))}
      ...
      {/* ---- Bento bottom : Alertes + Échéances ---- */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 items-start">
        ...
        <Card>
          <div className="flex items-center justify-between mb-4">
            <div className="sec-title">
              <Icon name="bell" size={17} className="text-text-3" />
              Alertes actives
            </div>
            {alerts.length > 0 && (
          ...
          {alerts.length === 0 ? (
            isNewUser ? (
              <div className="flex flex-col gap-2.5 py-1">
                <div className="text-text-2 text-sm mb-1">
                  Aucune alerte — commencez par ces étapes :
                </div>
                {setupSteps
                  .filter((s) => !s.done)
                  .slice(0, 3)
            ...
            <div>
              {alerts.map((a, i) => (
                <AlertItem key={i} alert={a} isLast={i === alerts.length - 1} />
              ))}
            </div>
          )}
        </Card>
        ...
        <Card>
          <div className="flex items-center justify-between mb-4">
            <div className="sec-title">
              <Icon name="clock" size={17} className="text-text-3" />
              Prochaines échéances
            </div>
            {upcoming.length > 0 && (
              <Link href="/payments" className="link text-[13px]">
                Voir tout
              </Link>
            )}
          </div>
          ...
          {upcoming.length === 0 ? (
            <div className="flex flex-col items-center gap-3 py-8 text-center">
              ...
              {state.tasks.length === 0 && (
                <Link href="/checklist">
                  <Button variant="secondary" size="sm" icon="check-circle">
                    Voir la checklist
                  </Button>
                </Link>
              )}
            ...
            <div>
              {upcoming.map((u, i) => (
                <DeadlineItem key={i} item={u} isLast={i === upcoming.length - 1} />
              ))}
            </div>
          )}
        </Card>
